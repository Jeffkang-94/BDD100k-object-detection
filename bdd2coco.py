...
dst_val_dir = os.path.join(cfg.bdd_dir, 'labels_coco', 'bdd100k_labels_images_' + scene +'_val_coco.json')
dst_train_dir = os.path.join(cfg.bdd_dir, 'labels_coco', 'bdd100k_labels_images_' + scene +'_train_coco.json')
...
def bdd2coco_detection(labeled_images, save_dir):
  ...
  for i in tqdm(labeled_images):
    ...
    tmp = 0
    if i['attributes']['timeofday'] == scene:
      for l in i['labels']:
          annotation = dict()
